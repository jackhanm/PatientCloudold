...
#define PICKER_MAX 2050
#define PICKER_MIN 1970
#define PICKER_MONTH 12
#define PICKER_HOUR 24
#define PICKER_MINUTE 60

#define YU_GRAY [UIColor redColor];
#define YU_BLACK [UIColor blackColor];
#ifndef isIOS7
#define isIOS7  ([[[UIDevice currentDevice]systemVersion]floatValue] >= 7.0)
#endif
@interface RPreportSearchView ()
{
    ...
    //限制model
    DataPickerModel *maxDateModel;
    DataPickerModel *minDateModel;
...
@implementation RPreportSearchView
...
- (void)drawRect:(CGRect)rect
{
    ...
    //赋值
    for (int i=0; i<PICKER_MINUTE; i++) {
        NSString *num = [NSString stringWithFormat:@"%02d",i];
        if (0<i && i<=PICKER_MONTH)
            [monthArray addObject:num];
        if (i<PICKER_HOUR)
            [hourArray addObject:num];
        [minuteArray addObject:num];
    }
    for (int i=PICKER_MIN; i<PICKER_MAX; i++) {
    ...
    if (self.maxLimitDate) {
        maxDateModel = [[DataPickerModel alloc]initWithDate:self.maxLimitDate];
    }else{
    ...
    if (self.minLimitDate) {
        minDateModel = [[DataPickerModel alloc]initWithDate:self.minLimitDate];
    }else{
...
- (NSArray *)getNowDate:(NSDate *)date
{
    ...
    DataPickerModel *model = [[DataPickerModel alloc]initWithDate:dateShow];
    
    [self DaysfromYear:[model.year integerValue] andMonth:[model.month integerValue]];
    
    yearIndex = [model.year intValue]-PICKER_MIN;
...
{
    if (self.datePickerStyle == UUDateStyle_YearMonthDayHourMinute){
        if (component == 0) return PICKER_MAX-PICKER_MIN;
        ...
        if (component == 3) return PICKER_HOUR;
        if (component == 4) return PICKER_MINUTE;
    }
    if (self.datePickerStyle == UUDateStyle_YearMonthDay)
    {
        if (component == 0) return PICKER_MAX-PICKER_MIN;
    ...
    if (self.datePickerStyle == UUDateStyle_MonthDayHourMinute)
    {
        if (component == 0) return PICKER_MONTH;
        if (component == 1){
            return [self DaysfromYear:[yearArray[yearIndex] integerValue] andMonth:[monthArray[monthIndex] integerValue]];
        }
        if (component == 2) return PICKER_HOUR;
        if (component == 3) return PICKER_MINUTE;
    }
    if (self.datePickerStyle == UUDateStyle_HourMinute)
    {
        if (component == 0) return PICKER_HOUR;
        else                return PICKER_MINUTE;
    }
    return 0;
}
